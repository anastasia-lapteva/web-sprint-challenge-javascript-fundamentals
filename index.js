...
// "nestedFunction()"" can access the variable "internal" because inner functions can see outer functions. If a function does not have a specific variable locally, it looks outside (globally) to look for that variable. In this specific case, "nestedFunction()" does not have a local variable "internal" so it look outside for it.
...
{
    const namesArray = array.map(animal => animal.animal_name.toLowerCase());
    return namesArray;
}
...
{
    const filterLowPop = array.filter((animal) => 
    {
...
console.log("Callback Divide:", consume(10, 5, divide));
...
console.log("Callback Add:", consume(5, 10, add));
...
console.log("Callback Multiply:", consume(5, 10, multiply));
...
console.log("Check Greeting:", consume("Mary", "Poppins", greeting)); // Hello Mary Poppins, nice to meet you!
...
const cubeMaker = new CubeMaker(5);
